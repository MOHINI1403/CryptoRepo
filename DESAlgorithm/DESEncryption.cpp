...
class DES_Encryption{
    const int IP_t[64] = { 	58 ,50 ,42 ,34 ,26 ,18 ,10 ,2 ,  // intital permutation table
                ...
                //In the Third Step of the Manglar Function we need to perform S-Box Subsitution 48bit input converted to 32 bit
                //Here 8 6-bit input block are considered
    int S[8][4][16] = {                        // S-box
		{
    ...
    //This will take n as by how many bits are we supposed to perform circular shift left

    string shift_bit(string s, int n)
	{
		string k = "";
        //If we want to perform the cirular shift left now firsly the bits from n to size are taken and then rest of the bits
    ...
    public:
        void encrypt(string plainText,string key){
            ...
            //First Step is to convert the 64 bit string taken as input into 56 bits string along with doing initial Permutaion
            string key_56 = "";
            ...
            /*First Step is Key Transformation and within key transformation the 48 bit key is generated using the 56 bit key 
            Depending on the Number of rounds the bits are shifted to left in a circular fashion
            */
            string L_key[16], R_key[16];
            // The first character of this 16 length L_Key and right key contains the 56 bit key 
            ...
            //Every value of the L_KEY and R_KEY is of 56 bits and for 16 rounds 16 possible 56 bit key are generated

            //Now the task is to convert this 56 bit key(i.e 28bit in each half) first combine them
            ...
            //Now this 56bit key converted into the 48bit key 
		    for (int i = 0; i < 16; i++)
		    {
			    key_48[i] = "";
			    for (int j = 0; j < 48; j++)
            ...
            string plain_txt_64 = Hex_to_Bin(plainText); // transforming key from 16-bits hex to 64-bits bin

            string IP = ""; // permuted key
            //Performing Initial Permutaion Before starting the Encryption Process
		    for (int i = 0; i < 64; i++)
			    IP += plain_txt_64[IP_t[i] - 1];

		    string L = "", R = "";
            //Dividing the 64bit input block into two halfes of 32 bit each
            ...
            //Left half of the plainText is left as it is 

            string S_R[16], s[16][8];
		    string s_1[16];
		    string P_R[16];

            //This R_48 will contain the expanded data from the 32 bit data for all 16 rounds
            R_48[0] = "";
		    for (int j = 0; j < 48; j++)
			    R_48[0] += R[E_t[j] - 1];
            ...
            //This R_xor_K contains the 48bit value for S-BOX Subsitution
            ...
            //We have 8 blocks and each block is of 6 character
            //We are supposed to run this loop 8 times since we have 8 blocks 

            //Every String of s_1 block is of 32 bits
            s_1[0] = "";
		    for (int j = 0; j < 8; j++)
			    s_1[0] += get_element_from_box(s[0][j], j);
            
            //Perform the step-4 i.e P-Box Permutation
